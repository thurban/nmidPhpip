...
$pdesc = mysql_query("SELECT * FROM `phpIP_NetMenu` WHERE `NetMenuId` = '$di'");
	$RowClass = "";
...
$updesc = mysql_query("UPDATE `phpIP_NetMenu` SET `NetCidrDescription` = '$de' WHERE `NetMenuId` = '$di'");
...
$pdesc = mysql_query("SELECT * FROM `phpIP_NetMenu` ORDER BY `NetMenuCidr` + 0");
	$RowClass = "";
    while ($row = mysql_fetch_array($pdesc))
        {
        if ($RowClass == "listRow2") { $RowClass = "listRow1";
