...
if (isset($_POST['prefix'])) { $i = 0;
  if (count($_POST['prefix']) > 0)
	  ...
	  $CidrRemove = mysql_query("DELETE FROM `phpIP_addresses` WHERE `ip` like '$prefixremove%'");
      //$CidrRemove = mysql_query("DELETE FROM `phpIP_net_ips` WHERE `NetID` = '$prefixremove'");
      $CidrRemove = mysql_query("UPDATE `phpIP_net_ips` SET `view` = '0' WHERE `netaddress` = '$prefixremove'");
	$i++;
  }
}
...
$sqllist = mysql_query("SELECT * FROM `phpIP_net_ips` where `view` = '1' ORDER BY `NetCidr` + 0");
?>
