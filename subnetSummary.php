...
$NetCount_sql = mysql_query("SELECT COUNT(*) AS NetCount FROM `phpIP_net_ips` WHERE `view` = '1'");
...
$NetListView = mysql_query("SELECT `netaddress`, `NetCidr` FROM `phpIP_net_ips` WHERE `view` = '1'");
...
$NetTotal_sql = mysql_query("SELECT COUNT(*) AS NetTotal FROM `phpIP_addresses` WHERE `ip` 
                                LIKE '%$iprangeEx[0].$iprangeEx[1].$iprangeEx[2].%' AND `NetID` = '".$row['NetCidr']."'");

// grab used number of address based on prefixes (netaddress)
$NetUsed_sql = mysql_query("SELECT COUNT(*) AS Used FROM `phpIP_addresses` WHERE `ip` 
...
<?php
$NetDisplay_sql = mysql_query("SELECT `netaddress` FROM `phpIP_net_ips` WHERE `view` = '1'");
